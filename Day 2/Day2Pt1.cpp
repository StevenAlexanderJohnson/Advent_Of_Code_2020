...
int main()
{
	...
	if(inputFile.is_open())
	{
		...
		for(int i = 0; i < numbers.size(); i++)
		{
			...
			// Getting value of the character.
			char letterCheck = *letter[i].c_str();
			// Getting value of password to check.
			string passwordCheck = password[i];
			
			// Counting how many times the character appears in the string.
			int letterCount = 0;
			for(int j = 0; j < passwordCheck.size(); j++)
			{
				if(passwordCheck[j] == letterCheck)
				{
					letterCount++;
				}
			}
			// If the letter count is within range given add it to valid passwords.
